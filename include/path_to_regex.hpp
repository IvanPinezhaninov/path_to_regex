...
static std::string percent_encode(std::string_view str)
{
...
static std::string percent_decode(std::string_view str)
{
  std::string decoded;
...
static char find_separator(std::string_view path)
{
  return (path.find('/') <= path.find('\\')) ? '/' : '\\';
}

static std::string make_pattern(const std::string& path, std::vector<std::string>& keys, char separator)
{
...
static std::string make_pattern(std::string_view path, std::vector<std::string>& keys)
{
...
static std::regex_constants::syntax_option_type make_regex_flags(path_to_regex::case_sensitivity sensitivity)
{
...
public:
  ...
  struct result {
    ...
    std::map<std::string, std::string> params; ///< Extracted params from the matched path.
  };
 ...
 * @see matcher
 */
static matcher match(std::string_view path, case_sensitivity sensitivity = case_sensitivity::case_sensitive)
{
