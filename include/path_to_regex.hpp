...
char find_separator(std::string_view path) { return (path.find('/') <= path.find('\\')) ? '/' : '\\'; }

std::string make_pattern(const std::string &path, std::vector<std::string> &keys, char separator)
{
...
std::string make_pattern(std::string_view path, std::vector<std::string> &keys)
{
...
std::regex_constants::syntax_option_type make_regex_flags(path_to_regex::case_sensitivity sensitivity)
{
 ...
 * @see matcher
 */
matcher match(std::string_view path, case_sensitivity sensitivity = case_sensitivity::case_sensitive)
{
